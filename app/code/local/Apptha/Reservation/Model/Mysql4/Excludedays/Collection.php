 ...
 */
class Apptha_Reservation_Model_Mysql4_Excludedays_Collection extends Mage_Core_Model_Mysql4_Collection_Abstract {
    ...
    public function getExcludeDates($strEntityId, $strPerType) {
        $this->getSelect()
                ->where('entity_id=' . $strEntityId);
        if ($strPerType == 'single') {
            $this->getSelect()->where('period_type = "single"');
        } else if ($strPerType == 'period')
            $this->getSelect()->where('period_type = "period"');
        return $this;
    }
