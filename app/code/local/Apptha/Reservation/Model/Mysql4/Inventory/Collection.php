 ...
 */
class Apptha_Reservation_Model_Mysql4_Inventory_Collection extends Mage_Core_Model_Mysql4_Collection_Abstract
 {
 ...
 /**
	...
	*/	
	public function addEntityIdFilter($id){
 	...
 	public function getRoomIdFilter($id){
 	...
 	public function getRoomPeriodIdFilter($id, $from, $productId, $orderId){
		 $this->getSelect()->where('room_id='.intval($id))
						->where('booked_date_timestamp='.intval($from))
