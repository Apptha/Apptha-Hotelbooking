...
class Apptha_Reservation_Block_Adminhtml_Catalog_Product_Edit_Tab_Rules_Roomtype
    extends Mage_Adminhtml_Block_Widget
    implements Varien_Data_Form_Element_Renderer_Interface
{
    public function __construct(){
        $this->setTemplate('reservation/product/edit/tab/rules/roomtype.phtml');
    }
    ...
    public function createDateBlock($name, $type){
        ...
        if($type == 'from'){
			$days_select = $this->getLayout()->createBlock('core/html_select')
					...
					->setId('special_days_row___index___recurrent_day')
					->setOptions(
						array(
							1 => $this->__('Monday'),
							2 => $this->__('Tuesday'),
			...
			$element
				->setName($name.'[__index__][room_period_from]')
				->setId('special_days_row___index___period_from')
			...
			$element
				->setName($name.'[__index__][room_period_to]')
				->setId('special_days_row___index___period_to') 
				->setClass('product-custom-option datetime-picker input-text require booking-date');
		}        
